...
//helper structure to store generation data
typedef struct 
{
	///name of the file
	string filename;
	///ISR type
	int    keyISR;
	/// 2 - convolution off; 3- convolution on
	int    kDim;
	/// energy spread
	double sigE;
	/// total number of events
	int    NevTot;
	/// 0-use Born convolution or 1-ISR convolution
	int	   BornConv;
} genData;
...
int main()
{
	...
	//fill list of generation 
	int nData = 13;
	genData data[nData];
	
	{
		int i = 0;
		data[i].filename = string("histo-sig04-isr2");
		data[i].keyISR   = 2;
		data[i].kDim     = 3;
		data[i].sigE     = 0.0042;
		data[i].NevTot   = NevTot;
		data[i].BornConv = 1;

		i = 1;
		data[i].filename = string("histo-sig08-isr2");
		data[i].keyISR   = 2;
		data[i].kDim     = 3;
		data[i].sigE     = 0.008;
		data[i].NevTot   = NevTot;
		...
		i = 2;
		data[i].filename = string("histo-sig15-isr2");
		data[i].keyISR   = 2;
		data[i].kDim     = 3;
		data[i].sigE     = 0.015;
		data[i].NevTot   = NevTot;
		...
		i = 3;
		data[i].filename = string("histo-sig30-isr2");
		data[i].keyISR   = 2;
		data[i].kDim     = 3;
		data[i].sigE     = 0.03;
		data[i].NevTot   = NevTot;
		...
		i = 4;
		data[i].filename = string("histo-sig100-isr2");
		data[i].keyISR   = 2;
		data[i].kDim     = 3;
		data[i].sigE     = 0.1;
		data[i].NevTot   = NevTot;
		...
		i = 5;
		data[i].filename = string("histo-sig0-isr0");
		data[i].keyISR   = 0;
		data[i].kDim     = 2;
		data[i].sigE     = 0.0050;
		data[i].NevTot   = NevTot;
		...
		i = 6;
		data[i].filename = string("histo-sig0-isr1");
		data[i].keyISR   = 1;
		data[i].kDim     = 2;
		data[i].sigE     = 0.0050;
		data[i].NevTot   = NevTot;
		...
		i = 7;
		data[i].filename = string("histo-sig0-isr2");
		data[i].keyISR   = 2;
		data[i].kDim     = 2;
		data[i].sigE     = 0.0050;
		data[i].NevTot   = NevTot;
		...
		i = 8;
		data[i].filename = string("histo-sig04-born");
		data[i].keyISR   = 0;
		data[i].kDim     = 0;
		data[i].sigE     = 0.0042;
		data[i].NevTot   = NevTot;
		...
		i = 9;
		data[i].filename = string("histo-sig08-born");
		data[i].keyISR   = 0;
		data[i].kDim     = 0;
		data[i].sigE     = 0.008;
		data[i].NevTot   = NevTot;
		...
		i = 10;
		data[i].filename = string("histo-sig15-born");
		data[i].keyISR   = 0;
		data[i].kDim     = 0;
		data[i].sigE     = 0.015;
		data[i].NevTot   = NevTot;
		...
		i = 11;
		data[i].filename = string("histo-sig30-born");
		data[i].keyISR   = 0;
		data[i].kDim     = 0;
		data[i].sigE     = 0.03;
		data[i].NevTot   = NevTot;
		...
		i = 12;
		data[i].filename = string("histo-sig100-born");
		data[i].keyISR   = 0;
		data[i].kDim     = 0;
		data[i].sigE     = 0.1;
		data[i].NevTot   = NevTot;
	...
	//for given process determine array bounds for it
	int ifirst = startIndex( nData, nproc, iproc );
	int ilast  = stopIndex( nData, nproc, iproc );
	
	for( int i = ifirst; i <= ilast; i++ )
	{
		
		#ifdef DEBUG
			cout << "Process " << iproc << " has range: " << ifirst << ", " << ilast << endl;
			cout << "Process " << iproc << " do " << data[i].filename << ", index " << i << endl;
		#endif	
		
		if( data[i].BornConv == 0 )
		{
			MakeConvBorn( data[i].filename, data[i].sigE, data[i].NevTot );
		}
		
		if( data[i].BornConv == 1 )
		{
			MakeISR( data[i].filename, data[i].keyISR, data[i].kDim, data[i].sigE, NevTot );
		}
