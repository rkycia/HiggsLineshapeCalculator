...
void plotISRdelta( void )
{
	...
	TCanvas* canv2 = new TCanvas("canv2","plot2");	
	canv2->SetLogy();
	canv2->cd(1);
  
	h_SigEne0->SetMaximum(10);
	h_SigEne0->GetYaxis()->SetTitle( "#sigma(s) [fb] (Logscale)" );
	h_SigEne0->Draw("l");
	h_SigEne04->Draw("lsame");
	h_SigEne08->Draw("lsame");
	h_SigEne15->Draw("lsame");
...
void plotBorndelta( void )
{
	...
	h_SigBorn->SetMaximum(10);
	h_SigBorn->GetYaxis()->SetTitle("#sigma(s) [fb] (Logscale)");
	h_SigBorn->Draw("l");
	h_SigEne04->Draw("lsame");
	h_SigEne08->Draw("lsame");
	h_SigEne15->Draw("lsame");
	...
	canv2->Print( "BornspreadLogY.eps", "" );		
...
int main()
{
	...
	cout << "ISR data..." << endl;	
		...
		//ISR, sig = 8 MeV, isr = 2, convolution
		MakeISR( string("histo-sig08-isr2"), 2, 3, 0.008, NevTot );
	
		//ISR, sig = 15 MeV, isr = 2, convolution
		MakeISR( string("histo-sig15-isr2"), 2, 3, 0.015, NevTot );
	
		//ISR, sig = 30 MeV, isr = 2, convolution
		MakeISR( string("histo-sig30-isr2"), 2, 3, 0.030, NevTot );
	
		//ISR, sig = 100 MeV, isr = 2, convolution
		MakeISR( string("histo-sig100-isr2"), 2, 3, 0.1, NevTot );
	
		//ISR, isr = 0, NO convolution
		MakeISR( string("histo-sig0-isr0"), 0, 2, 0.0050, NevTot );
	
		//ISR, isr = 1, NO convolution
		MakeISR( string("histo-sig0-isr1"), 1, 2, 0.0050, NevTot );

		//ISR, isr = 2, NO convolution
		MakeISR( string("histo-sig0-isr2"), 2, 2, 0.0050, NevTot );
	...
	cout << "Born convolution data... " << endl;
		...
		//Born sig = 8 MeV, convolution 
		MakeConvBorn( string("histo-sig08-born"), 0.008, NevTot );
	
		//Born sig = 15 MeV, convolution 
		MakeConvBorn( string("histo-sig15-born"), 0.015, NevTot );
	
		//Born sig = 30 MeV, convolution 
		MakeConvBorn( string("histo-sig30-born"), 0.03, NevTot );
	
		//Born sig = 100 MeV, convolution 
		MakeConvBorn( string("histo-sig100-born"), 0.1, NevTot );
	...
	cout << "sigE data..." << endl;
		...
		makeISRsigEDistribution( 50, NevTot );//poduction
		//makeISRsigEDistribution( 10, NevTot );//testing
	
	cout << "...sigE data DONE" << endl;
